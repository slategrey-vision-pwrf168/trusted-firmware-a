...
int DecodeMessage(uint32_t tag, const UsefulBufC encodedBuf, UsefulBuf *messageBuf)
{
  ...
  if (item.val.string.len > messageBuf->len)
  {
    ...
    INFO("%s: EXIT ret=%d (response %zu exceeds capacity %zu)\n", __func__,
         E_NO_MEMORY, item.val.string.len, messageBuf->len);
    messageBuf->len = 0;
    return E_NO_MEMORY;
  }
                                     ...
                                     uint32_t  timeoutMSec)
{
      ...
      (tmecomMsgRspLen > (sizeof(tmecomMsgHdr) + encodedResponseLen)))
  {
    ...
    INFO("%s: implausible response length %zu (hdr:%zu max:%zu)\n", __func__,
         tmecomMsgRspLen, sizeof(tmecomMsgHdr),
         sizeof(tmecomMsgHdr) + encodedResponseLen);
    ret = E_FAILURE;
    goto exit;
  }
